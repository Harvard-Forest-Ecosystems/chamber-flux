...
format_LGR_output <- function(data_path){
  ...
  list_of_zip_files <- list.files(path = data_path, recursive=TRUE, 
  ...
  list_of_txt_files <- list.files(path = data_path, recursive = TRUE,
